use sea_orm::{
    ColumnTrait, DatabaseConnection, DbBackend, DbErr, EntityTrait, JoinType, QueryFilter,
    QuerySelect, RelationTrait, Set, Statement,
};

use crate::repo::rdbms::custom_models::CharacterOverview;
use crate::{
    entity::character::{self, Entity as Character},
    entity::class,
};
...
impl CharacterRepo {
    ...
    pub async fn get_view_by_id(
        db: &DatabaseConnection,
        id: i32,
    ) -> Result<Option<CharacterOverview>, DbErr> {
    ...
    pub async fn create(
        ...
        let result = Character::insert(character::ActiveModel {
    ...
    pub async fn delete_by_id(db: &DatabaseConnection, id: i32) -> Result<(), DbErr> {
        Character::delete_by_id(id).exec(db).await?;
